...
const SpiderChart = ({
  ...
  const [data, setData] = useState({
    labels: ["Analys", "Fart", "Prestation", "Motstånd"],
    datasets: [],
  });
  ...
  const getLegendPosition = () =>
    window.innerWidth >= 640 ? "right" : "top";
  ...
  useEffect(() => {
    ...
    fetch(
      ...
      .then((data) => {
        setLaps(data);
        const lapExists = data.some((l) => l.id === +selectedLap);
  ...
  const htmlLegendPlugin = {
    ...
    afterUpdate(chart) {
      ...
      chart.data.datasets.forEach((ds, idx) => {
        ...
        const visible = chart.isDatasetVisible(idx); //Changed!
        li.style.opacity = visible ? 1 : 0.35; //Changed!

        li.onclick = () => {
          chart.setDatasetVisibility(idx, !chart.isDatasetVisible(idx)); //Changed!
  ...
  /* ---------- helpers for Swedish date strings ---------- */
  ...
  const selectedDateLabel =
    selectedDate === todayStr
      ? `Idag, ${formatSE(selectedDate)}`
      : selectedDate === yesterdayStr
      ? `Igår, ${formatSE(selectedDate)}`
      : selectedDate === tomorrowStr
      ? `Imorgon, ${formatSE(selectedDate)}`
      : formatSE(selectedDate);
  ...
  return (
    <div className="flex flex-col justify-center items-center mt-1 px-2 pb-10">
      ...
      <ul
        ...
        className={`${
          isSmallScreen
            ? "relative z-10 grid grid-cols-3 gap-x-2 gap-y-1 text-xs"
            : "hidden"
        }`}
      />
